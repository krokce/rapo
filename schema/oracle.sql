...
-- Log lines emitted by the PL-SQL engine (RAPO_USAGE_RULE) while it runs.
-- The control process drains them into the run's own log file, so they are
-- transport only: rows are deleted as they are consumed.
